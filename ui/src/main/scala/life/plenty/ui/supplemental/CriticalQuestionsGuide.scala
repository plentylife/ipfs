...
object CriticalQuestionsGuide {
  ...
  private def filterSingleCritical(q: Question): Rx[Option[Question]] = {
    val hasCreated = DeprecatedGraphExtractors.getAllCreatedByInSpace(q, UiContext.getUser) // shouldn't be null
    val isFinalized = DeprecatedGraphExtractors.markedConfirmed(q)
    Rx {
